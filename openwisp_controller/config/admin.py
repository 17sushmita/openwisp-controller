...
class SystemDefinedVariableField(object):
    def system_context(self, obj):
        ...
        for key, value in system_context.items():
            if '\n' in value:
                css = 'multi'
                html = f'<span class="{css}"><pre> {value}</pre></span>'
            else:
                html = f'<span class="{css}"> {value}</span>'
            output += f'<p><strong>{key}: </strong> {html}</p>'
        output += (
            f'<textarea name="system_context" id="system_context" style="display:none">'
            f'{json.dumps(system_context)}</textarea>'
        )
...
class TemplateAdmin(MultitenantAdminMixin, BaseConfigAdmin, SystemDefinedVariableField):
    form = TemplateForm
    list_display = [
        'name',
        'organization',
...
class VpnAdmin(
    MultitenantAdminMixin, BaseConfigAdmin, UUIDAdmin, SystemDefinedVariableField
):
    form = VpnForm
